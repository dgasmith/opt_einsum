...
def path_cost(inputs, output, size_dict, path):
    """Computes the cost (=number of operations required) of a contraction path.
    ...
    -------
    cost : int
        Number of operations required for the given contraction path.

    Examples
    --------
    >>> path_cost([set("ik"), set("kj")], set("ij"), {k: 3 for k in "ijk"}, [(0, 1)])
    27
    """
    c = 0
    for p in path:
        inputs_to_contract = [i for j, i in enumerate(inputs) if j in p]
        i_inputs_to_contract = set.union(*inputs_to_contract)
        c += compute_size_by_dict(i_inputs_to_contract, size_dict)
        inputs = [i for j, i in enumerate(inputs) if j not in p]
        i_contract = i_inputs_to_contract - output
        if len(inputs) > 0:
            i_contract -= set.union(*inputs)
        new_input = i_inputs_to_contract - i_contract
        inputs += [new_input]
    return c
